...
(function(){
  ...
  class MadarP2P {
    ...
    async start(parent){
      ...
      this.started=true; this.poll(); this.pollTimer=setInterval(()=>this.poll(),1600);
    ...
    async updateState(){
      ...
      this.addTile('local', this.room.userName+' (شما)', this.screenStream||this.localStream, true, !!this.room.isHost, {user_id:this.room.userId,mic_on:this.micOn?1:0,cam_on:this.camOn?1:0,screen_on:this.screenOn?1:0,is_host:this.room.isHost?1:0});
    ...
    updatePeerMeta(peer){ const st=this.peers.get(peer.peer_id); if(st) st.peer=Object.assign(st.peer||{},peer); const tile=this.tiles.get(peer.peer_id); if(tile) this.applyTileState(tile, peer); }
    async createPeer(peer, politeOffer){
      ...
      pc.ontrack=ev=>{ const st=this.peers.get(peer.peer_id); if(!st.remoteStream) st.remoteStream=new MediaStream(); ev.streams[0].getTracks().forEach(t=>{ if(!st.remoteStream.getTracks().includes(t)) st.remoteStream.addTrack(t); }); this.addTile(peer.peer_id, peer.name||'کاربر', st.remoteStream, false, !!peer.is_host, peer); };
    ...
    async post(action, body){ const res=await fetch(`${this.api}?action=${encodeURIComponent(action)}&room_id=${this.room.sessionId}`,{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify(body||{})}); return res.json(); }
    ...
    layout(){ if(!this.grid) return; const n=this.tiles.size; const focused=this.grid.classList.contains('has-focused'); this.grid.className='p2p-grid count-'+Math.min(6,Math.max(1,n))+(focused?' has-focused':''); const tiles=[...this.tiles.values()]; tiles.forEach(t=>t.classList.remove('is-main')); const main=tiles.find(t=>t.classList.contains('is-screen'))||tiles.find(t=>t.classList.contains('is-host'))||tiles[0]; main&&main.classList.add('is-main'); }
    ...
    async toggleScreen(){ if(!this.room.permissions.screen && !this.room.isHost) return false; if(this.screenStream){ await this.stopScreen(); return false; } try{ this.screenStream=await navigator.mediaDevices.getDisplayMedia({video:true,audio:false}); const screenTrack=this.screenStream.getVideoTracks()[0]; for(const {pc} of this.peers.values()){ const sender=pc.getSenders().find(s=>s.track&&s.track.kind==='video'); if(sender) sender.replaceTrack(screenTrack); else pc.addTrack(screenTrack,this.screenStream); } this.screenOn=true; const tile=this.tiles.get('local'); if(tile) tile.querySelector('video').srcObject=this.screenStream; await this.updateState(); screenTrack.onended=()=>this.stopScreen(); return true; }catch(e){ this.hooks.toast&&this.hooks.toast('اشتراک صفحه لغو شد یا پشتیبانی نمی‌شود.'); return false; } }
    async stopScreen(){ const camTrack=this.localStream.getVideoTracks()[0]||null; for(const {pc} of this.peers.values()){ const sender=pc.getSenders().find(s=>s.track&&s.track.kind==='video'); if(sender) sender.replaceTrack(camTrack); } if(this.screenStream){this.screenStream.getTracks().forEach(t=>t.stop()); this.screenStream=null;} this.screenOn=false; const tile=this.tiles.get('local'); if(tile) tile.querySelector('video').srcObject=this.localStream; await this.updateState(); }
