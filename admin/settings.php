...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($act==='add_subject') {
        ...
        if (!settings_table_ready()) {
            ...
            $lineNumber = preg_replace('/[^0-9]/', '', (string)($_POST['sms_line_number'] ?? ''));
            $templateId = preg_replace('/[^0-9]/', '', (string)($_POST['sms_template_id'] ?? ''));
            $paramDate = trim((string)($_POST['sms_param_date'] ?? 'DATE')) ?: 'DATE';
            $paramTime = trim((string)($_POST['sms_param_time'] ?? 'TIME')) ?: 'TIME';
            set_advisor_setting((int)$u['id'], 'sms_enabled', $enabled);
            set_advisor_setting((int)$u['id'], 'sms_api_key', $apiKey);
            set_advisor_setting((int)$u['id'], 'sms_line_number', $lineNumber);
            set_advisor_setting((int)$u['id'], 'sms_template_id', $templateId);
            set_advisor_setting((int)$u['id'], 'sms_param_date', $paramDate);
            set_advisor_setting((int)$u['id'], 'sms_param_time', $paramTime);
            log_activity((int)$u['id'], 'settings_updated', 'system', null, ['عملیات' => 'ذخیره تنظیمات پیامک sms.ir', 'فعال' => $enabled === '1' ? 'بله' : 'خیر', 'خط' => $lineNumber]);
            flash('success','تنظیمات پیامک ذخیره شد.');
        }
    }
...
<!-- ===== SMS SETTINGS ===== -->
<div class="panel mb-4" style="background:linear-gradient(160deg,var(--card),var(--surface));border:1px solid rgba(107,136,114,.34);padding:28px;border-radius:var(--r-lg);box-shadow:0 10px 30px rgba(0,0,0,.25)">
  <div class="panel-head between wrap gap-3 mb-4" style="align-items:center;border-bottom:1px solid var(--border-soft);padding-bottom:16px">
    <div class="flex gap-3" style="align-items:center">
      <span class="icon-tile" style="width:48px;height:48px;background:var(--sage-glass);color:var(--sage-light)"><?= icon('phone',24) ?></span>
      <div>
        <h3 style="font-size:1.25rem;font-weight:900;color:var(--text-1);margin:0">تنظیمات پیامک sms.ir</h3>
        <p class="muted mt-1" style="font-size:.86rem">برای ارسال پیامک رسمی زمان‌بندی جلسات، کلید API و شماره خط ارسال‌کننده را وارد کنید.</p>
      </div>
    </div>
    <span class="badge <?= ($pset['sms_enabled'] ?? '0') === '1' && !empty($pset['sms_api_key']) && !empty($pset['sms_line_number']) ? 'badge-sage' : '' ?>" style="font-weight:900">
      <?= (($pset['sms_enabled'] ?? '0') === '1' && !empty($pset['sms_api_key']) && !empty($pset['sms_line_number'])) ? 'فعال' : 'غیرفعال' ?>
    </span>
  </div>

  <form method="post" autocomplete="off">
    <?= csrf_field() ?><input type="hidden" name="s_action" value="sms_settings">
    <div class="grid gap-3" style="grid-template-columns:repeat(auto-fit,minmax(240px,1fr))">
      <div class="field panel" style="background:var(--surface-1);padding:16px;border-radius:12px;margin:0">
        <label style="font-weight:900;color:var(--text-1)">کلید API</label>
        <input class="input mt-2" name="sms_api_key" type="password" dir="ltr" value="<?= e($pset['sms_api_key'] ?? '') ?>" placeholder="X-API-KEY" autocomplete="new-password" style="font-family:monospace;font-size:.95rem">
        <span class="hint muted mt-2" style="font-size:.78rem">کلید API پنل sms.ir که در هدر X-API-KEY ارسال می‌شود.</span>
      </div>
      <div class="field panel" style="background:var(--surface-1);padding:16px;border-radius:12px;margin:0">
        <label style="font-weight:900;color:var(--text-1)">شماره خط ارسال‌کننده</label>
        <input class="input mt-2" name="sms_line_number" type="text" inputmode="numeric" dir="ltr" value="<?= e($pset['sms_line_number'] ?? '') ?>" placeholder="مثلاً 300000000000" style="font-family:monospace;font-size:.95rem">
        <span class="hint muted mt-2" style="font-size:.78rem">همان lineNumber پنل sms.ir، فقط با اعداد انگلیسی وارد شود.</span>
      </div>
      <div class="field panel" style="background:var(--surface-1);padding:16px;border-radius:12px;margin:0">
        <label style="font-weight:900;color:var(--text-1)">شناسه قالب خدماتی</label>
        <input class="input mt-2" name="sms_template_id" type="text" inputmode="numeric" dir="ltr" value="<?= e($pset['sms_template_id'] ?? '') ?>" placeholder="Template ID" style="font-family:monospace;font-size:.95rem">
        <span class="hint muted mt-2" style="font-size:.78rem">برای ارسال به شماره‌های لیست سیاه، در sms.ir یک قالب خدماتی با متغیرهای تاریخ و ساعت بسازید و شناسه آن را وارد کنید.</span>
      </div>
      <div class="panel" style="background:var(--surface-1);padding:16px;border-radius:12px;margin:0">
        <label style="font-weight:900;color:var(--text-1)">نام متغیرهای قالب</label>
        <div class="grid gap-2 mt-2" style="grid-template-columns:1fr 1fr">
          <input class="input" name="sms_param_date" dir="ltr" value="<?= e($pset['sms_param_date'] ?? 'DATE') ?>" placeholder="DATE" style="font-family:monospace">
          <input class="input" name="sms_param_time" dir="ltr" value="<?= e($pset['sms_param_time'] ?? 'TIME') ?>" placeholder="TIME" style="font-family:monospace">
        </div>
        <span class="hint muted mt-2" style="font-size:.78rem">اگر قالب شما متغیرهایی مثل DATE و TIME دارد، همین نام‌ها را وارد کنید.</span>
      </div>
      <div class="panel" style="background:var(--surface-1);padding:16px;border-radius:12px;margin:0;display:flex;flex-direction:column;justify-content:space-between;gap:14px">
        <label class="toggle-row between" style="align-items:center;margin:0">
          <span><b style="color:var(--text-1)">فعال‌سازی ارسال پیامک</b><br><small class="muted">پس از تأیید نهایی پیش‌نویس جلسه، پیامک ارسال می‌شود.</small></span>
          <label class="switch"><input type="checkbox" name="sms_enabled" value="1" <?= ($pset['sms_enabled'] ?? '0') === '1' ? 'checked' : '' ?>><span class="slider"></span></label>
        </label>
        <button type="submit" class="btn btn-gold btn-lg btn-block" style="font-weight:900"><?= icon('check',18) ?> ذخیره تنظیمات پیامک</button>
      </div>
    </div>
    <div class="mt-3" style="padding:12px 14px;background:rgba(217,178,95,.10);border:1px solid rgba(217,178,95,.28);border-radius:12px;color:var(--text-2);font-size:.84rem;line-height:1.9">
      <b style="color:var(--gold-light)">قالب خدماتی پیشنهادی در sms.ir:</b><br>
      با سلام و احترام، جلسه‌ی مشاوره شما در سامانه مَدار تنظیم شد. تاریخ: #DATE# ساعت: #TIME# لطفاً پنل کاربری خود را در مَدار بررسی فرمایید. madaar-edu.ir
    </div>
  </form>
</div>
