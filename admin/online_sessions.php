...
if ($action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $perms = [
            'mic' => (int)($_POST['allow_mic'] ?? 1),
            'cam' => (int)($_POST['allow_cam'] ?? 1),
            'screen' => (int)($_POST['allow_screen'] ?? 1),
            'whiteboard' => (int)($_POST['allow_whiteboard'] ?? 1),
...
<div class="os-form">
  ...
  <form method="post">
    ...
    <div class="os-section">
      ...
      <div class="os-perms">
        <label class="os-perm">
          <input type="checkbox" name="allow_mic" value="1" checked>
          <span>🎤 میکروفون</span>
        </label>
        <label class="os-perm">
          <input type="checkbox" name="allow_cam" value="1" checked>
          <span>📹 دوربین</span>
        </label>
        <label class="os-perm">
          <input type="checkbox" name="allow_screen" value="1" checked>
          <span>🖥️ اشتراک صفحه</span>
        </label>
        <label class="os-perm">
          <input type="checkbox" name="allow_whiteboard" value="1" checked>
      ...
      ?>
        <div class="os-session-card is-<?= $status ?>">
          ...
          <div class="os-sc-actions">
            <?php if ($status === 'live'): ?>
              <a href="online_room.php?session=<?= (int)$s['id'] ?>" class="btn btn-gold" style="flex:2;background:var(--success);color:#0c1512">
            ...
            <?php elseif ($status === 'ended'): ?>
              <a href="online_room.php?session=<?= (int)$s['id'] ?>" class="btn btn-ghost btn-block" style="color:var(--text-3)">
