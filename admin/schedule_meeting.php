    ...
    <form method="post" id="meetingForm">
      ...
      <div class="sm-section">
        ...
        <p class="help muted" style="font-size:.78rem;margin-top:8px">
          <?= icon('info',13) ?> تاریخ و ساعت اجباری هستند. ساعت توافقی با خالی گذاشتن ساعت نمایش داده می‌شود ولی پیشنهاد می‌شود همیشه ساعت دقیق وارد کنید.
        </p>
      </div>
      ...
      <div class="session-row <?= $isToday ? 'is-today' : '' ?>" style="<?= $s['status']==='cancelled' ? 'opacity:.5' : '' ?>">
        ...
        <div class="sr-info">
          <div class="sr-title">
            ...
            <?php if ($hasPhone): ?><span style="color:var(--sage);font-size:14px" title="پیامک ارسال شد">📱</span><?php endif; ?>
          ...
          <div class="sr-meta">
            ...
            <span><?= icon('clock',13) ?> <?= $s['session_time'] ? fa_num(substr((string)$s['session_time'], 0, 5)) : 'ساعت توافقی 🕒' ?></span>
...
function buildSmsMessage(){
  ...
  const title = document.getElementById('title')?.value?.trim() || (type === 'class' ? 'کلاس درسی' : 'جلسه‌ی مشاوره');
  const date = document.getElementById('session_date')?.value || '';
  const time = document.getElementById('session_time')?.value || '';

  const typeText = type === 'class' ? 'کلاس درسی' : 'جلسه‌ی مشاوره';
  const persianDate = formatPersianDate(date);
  const persianTime = time ? formatTime(time) : 'ساعت توافقی';
