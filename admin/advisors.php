...
$pageCatalog = advisor_page_catalog();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 6. تنظیم دسترسی صفحه‌های پنل
    if ($action === 'assign_pages') {
        $advisorId = (int)($_POST['advisor_id'] ?? 0);
        $pageKeys = $_POST['page_keys'] ?? [];
        if (!is_array($pageKeys)) $pageKeys = [];
        $validKeys = array_keys(advisor_page_catalog());
        $pageKeys = array_values(array_unique(array_filter(array_map('strval', $pageKeys), fn($k) => in_array($k, $validKeys, true))));

        if ($advisorId && $advisorId !== $adminId && empty($pageKeys)) {
            $err = 'حداقل یک صفحه باید برای مشاور انتخاب شود.';
        } elseif ($advisorId && $advisorId !== $adminId) {
            $targetRole = db()->prepare("SELECT role FROM users WHERE id=? AND role='advisor'");
            $targetRole->execute([$advisorId]);
            if ($targetRole->fetchColumn()) {
                db()->beginTransaction();
                try {
                    db()->prepare("DELETE FROM advisor_page_access WHERE advisor_id = ?")->execute([$advisorId]);
                    if (!empty($pageKeys)) {
                        $ins = db()->prepare("INSERT INTO advisor_page_access (advisor_id, page_key) VALUES (?, ?)");
                        foreach ($pageKeys as $key) $ins->execute([$advisorId, $key]);
                    }
                    db()->commit();
                    log_activity($adminId, 'تنظیم دسترسی صفحات مشاور', 'user', $advisorId, ['صفحات مجاز' => $pageKeys ?: ['همه صفحات']]);
                    $msg = 'دسترسی صفحه‌های مشاور با موفقیت ذخیره شد.';
                } catch (Throwable $e) {
                    db()->rollBack();
                    $err = 'خطا در ثبت دسترسی صفحه‌ها.';
                }
            }
        }
    }
...
$advisors = db()->query("
    SELECT u.id, u.full_name, u.username, u.email, u.phone, u.field, u.status, u.access_mode, u.role, u.created_at,
           EXISTS(SELECT 1 FROM advisor_settings s WHERE s.advisor_id=u.id AND s.skey='chat_only_user' AND s.svalue='1') AS is_chat_guest,
...
$pageAssignmentRows = db()->query("SELECT advisor_id, page_key FROM advisor_page_access")->fetchAll();
$pageAssignments = [];
foreach ($pageAssignmentRows as $row) {
    $pageAssignments[(int)$row['advisor_id']][] = (string)$row['page_key'];
}

$realAdvisors = array_values(array_filter($advisors, fn($a) => empty($a['is_chat_guest'])));
$totalAdv = count($realAdvisors);
$activeAdv = count(array_filter($realAdvisors, fn($a) => $a['status'] === 'active'));
$restrictedAdv = count(array_filter($realAdvisors, fn($a) => $a['access_mode'] === 'restricted'));
$totalStudentsManaged = array_sum(array_column($realAdvisors, 'student_count'));
...
.page-access-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px;max-height:420px;overflow:auto;padding:4px}
.page-access-card{display:flex;align-items:center;gap:10px;padding:13px 14px;border-radius:16px;background:var(--surface-2);border:1px solid var(--border-soft);cursor:pointer;transition:.16s var(--ease)}
.page-access-card:hover{border-color:var(--gold);background:var(--gold-glass)}
.page-access-card:has(input:checked){border-color:var(--sage);background:linear-gradient(135deg,rgba(107,136,114,.18),rgba(203,172,128,.08));box-shadow:0 0 0 1px rgba(107,136,114,.18) inset}
.page-access-card input{width:20px;height:20px;accent-color:var(--sage);flex-shrink:0}.page-access-card .pa-ico{width:34px;height:34px;border-radius:11px;background:var(--surface-3);display:grid;place-items:center;color:var(--gold-light);flex-shrink:0}.page-access-card b{font-size:.92rem;color:var(--text-1)}.page-access-card small{display:block;font-size:.72rem;color:var(--text-3);margin-top:2px}.page-access-actions{display:flex;gap:8px;flex-wrap:wrap;align-items:center;justify-content:space-between;padding:12px 14px;background:var(--surface-1);border:1px solid var(--border-soft);border-radius:16px}
    ...
    <table class="tbl" id="advisorsTable" style="min-width: 880px;">
      <thead>
        <tr style="background: var(--surface-2); border-bottom: 2px solid var(--border);">
          ...
          <th style="padding: 16px 24px; text-align: center; font-size: .9rem;">صفحه‌ها</th>
      ...
      <tbody>
        <?php if (empty($advisors)): ?>
          <tr>
            <td colspan="8" class="text-c muted" style="padding: 64px 24px;">
        ...
        <?php else: ?>
          <?php foreach ($advisors as $adv): 
              ...
              $allowedPages = $pageAssignments[(int)$adv['id']] ?? [];
              $hasCustomPages = !empty($allowedPages);
              $advJson = json_encode([
                  'id' => $adv['id'],
                  ...
                  'has_custom_pages' => $hasCustomPages,
                  'is_chat_guest' => !empty($adv['is_chat_guest'])
          ...
          <tr class="advisor-row" 
            ...
            <td style="padding: 18px 24px;">
              <div class="u-row">
                ...
                <div>
                  <div style="font-weight: 800; font-size: 1.1rem; color: var(--text);display:flex;align-items:center;gap:6px;flex-wrap:wrap"><?= e($adv['full_name']) ?><?php if(!empty($adv['is_chat_guest'])): ?><span class="adv-badge info" style="font-size:.66rem;padding:2px 8px">مهمان چت</span><?php endif; ?></div>
            ...
            <td style="padding: 18px 24px; text-align: center;">
              <?php if(!empty($adv['is_chat_guest'])): ?>
                <span class="adv-badge info" style="padding: 6px 14px"><?= icon('message',16) ?> فقط چت</span>
              <?php else: ?>
            ...
            <td style="padding: 18px 24px; text-align: center;">
              <?php if (!empty($adv['is_chat_guest'])): ?>
                <span class="adv-badge info" style="padding: 6px 14px;"><?= icon('message',16) ?> پیام‌ها</span>
              <?php elseif ($adv['access_mode'] === 'all'): ?>
            ...
            <td style="padding: 18px 24px; text-align:center;">
              <?php if ($adv['role'] === 'admin'): ?>
                <span class="adv-badge gold"><?= icon('shield',14) ?> همه صفحات</span>
              <?php elseif (!$hasCustomPages): ?>
                <button onclick='openPageAccessModal(<?= $advJson ?>)' class="adv-badge gold" style="cursor:pointer;border:none"><?= icon('globe',14) ?> همه صفحات</button>
              <?php else: ?>
                <button onclick='openPageAccessModal(<?= $advJson ?>)' class="adv-badge info" style="cursor:pointer;border:none"><?= icon('grid',14) ?> <?= fa_num(count($allowedPages)) ?> صفحه</button>
              <?php endif; ?>
            </td>
            ...
            <td style="padding: 18px 24px; text-align: center;">
              <div class="center gap-2 wrap">
                ...
                <?php if ($adv['role'] === 'advisor'): ?>
                <button onclick='openPageAccessModal(<?= $advJson ?>)' class="action-btn sage" data-tip="دسترسی صفحه‌ها">
                  <?= icon('grid', 18) ?>
                  <span>صفحه‌ها</span>
                </button>
                <?php endif; ?>
    ...
    <form method="post" class="modal-custom-body grid gap-4">
      ...
      <div class="grid gap-4" style="grid-template-columns: 1fr 1fr;">
        <div class="field m-0">
          ...
          <input type="text" name="full_name" class="input" placeholder="مثال: دکتر محمد کریمی" required>
        </div>
        <div class="field m-0">
      ...
      <div class="grid gap-4" style="grid-template-columns: 1fr 1fr;">
        <div class="field m-0">
          ...
          <input type="text" name="username" class="input" dir="ltr" placeholder="karimi_adv" required>
        </div>
        <div class="field m-0">
...
<!-- 4. Modal: مدیریت دسترسی صفحه‌های پنل مشاور -->
<div class="modal-backdrop" id="pageAccessModal">
  <div class="modal" style="max-width: 880px; padding: 0; overflow: hidden; background: var(--surface);">
    <div class="between" style="padding: 24px 32px; background: var(--surface-2); border-bottom: 1px solid var(--border);">
      <h3 style="font-size: 1.35rem; color: var(--sage-light); font-weight: 900; display: flex; align-items: center; gap: 10px;">
        <?= icon('grid', 26) ?> <span id="pageAccessModalTitle">دسترسی صفحه‌ها</span>
      </h3>
      <button class="modal-close" onclick="closeModal('pageAccessModal')" style="width: 36px; height: 36px;"><?= icon('close', 18) ?></button>
    </div>

    <form method="post" class="modal-custom-body grid gap-4">
      <input type="hidden" name="action" value="assign_pages">
      <input type="hidden" name="advisor_id" id="page_access_advisor_id" value="">
      <?= csrf_field(); ?>

      <div class="page-access-actions">
        <div>
          <b style="color:var(--text-1)">صفحه‌های مجاز برای این مشاور</b>
          <div class="muted" style="font-size:.82rem;margin-top:3px">اگر همه گزینه‌ها انتخاب شوند، مشاور عملاً به همه صفحه‌های پنل دسترسی دارد.</div>
        </div>
        <div class="flex gap-2 wrap">
          <button type="button" onclick="selectAllPages(true)" class="btn btn-sage btn-sm">انتخاب همه</button>
          <button type="button" onclick="selectAllPages(false)" class="btn btn-ghost btn-sm" style="color:var(--danger)">لغو همه</button>
        </div>
      </div>

      <div class="page-access-grid">
        <?php foreach ($pageCatalog as $key => $page): ?>
          <label class="page-access-card">
            <input type="checkbox" name="page_keys[]" value="<?= e($key) ?>" class="page-access-checkbox">
            <span class="pa-ico"><?= icon($page['icon'],18) ?></span>
            <span><b><?= e($page['label']) ?></b><small><?= e($page['path']) ?></small></span>
          </label>
        <?php endforeach; ?>
      </div>

      <div class="between pt-4 mt-2" style="border-top: 1px solid var(--border-soft); justify-content: flex-end; gap: 12px;">
        <button type="button" onclick="closeModal('pageAccessModal')" class="btn btn-ghost px-6">انصراف</button>
        <button type="submit" class="btn btn-sage px-8 py-3" style="font-weight: 900;">ذخیره دسترسی صفحه‌ها</button>
      </div>
    </form>
  </div>
</div>
...
function openPageAccessModal(adv) {
    document.getElementById('page_access_advisor_id').value = adv.id;
    document.getElementById('pageAccessModalTitle').innerText = `دسترسی صفحه‌ها • ${adv.full_name}`;
    const allowed = adv.allowed_pages || [];
    const hasCustom = !!adv.has_custom_pages;
    const boxes = document.querySelectorAll('.page-access-checkbox');
    boxes.forEach(cb => {
        cb.checked = hasCustom ? allowed.includes(cb.value) : true;
    });
    document.getElementById('pageAccessModal').classList.add('open');
}

function selectAllPages(select) {
    document.querySelectorAll('.page-access-checkbox').forEach(cb => { cb.checked = select; });
}
