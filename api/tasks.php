...
case 'set_status': {
    ...
    // فقط وقتی تمام تسک‌های امروز تعیین‌وضعیت شدن → پیشنهاد گزارش امروز
    $todayReport = null;
    $todayDate = date('Y-m-d');
    [$todayStart,$todayEnd] = report_period('daily', $todayDate);
    if (report_period_has_tasks($me, $todayStart, $todayEnd)
        && report_period_is_closed($me, $todayStart, $todayEnd)
        && !report_is_submitted($me, 'daily', $todayStart)) {
        $todayReport = ['url' => url('student/reports.php?type=daily&date='.$todayStart), 'label' => 'روزانه'];
    }
    json_out(['ok'=>true,'status'=>$status,'is_done'=>$isDone,'done_count'=>$doneCount,'course_percent'=>$coursePercent,
        ...
        'needs_report'=>(bool)$todayReport,'report_url'=>$todayReport['url'] ?? null,'report_label'=>$todayReport['label'] ?? null]);
}
...
/* ============ لایو استاتوس — برای داشبورد ============ */
case 'live_status': {
    if ($role !== 'student') json_out(['ok'=>false,'error'=>'فقط دانش‌آموز'],403);
    $user = get_user($me);
    $todayTasks = student_today_tasks($me);
    $total = count($todayTasks);
    $score = 0; $done = 0; $partial = 0;
    ...
    json_out([
        ...
        'today_total' => $total,
        'unread_notif' => unread_notif_count($me),
        'unread_msg' => unread_msg_count($me),
    ]);
}
